...
sp_nativeinfo_t g_ExtensionNatives[] =
{
	...
	{ "TF2Items_SetItemId", TF2Items_SetItemId },
	{ "TF2Items_GetItemId", TF2Items_GetItemId },
	{ "TF2Items_CopyDynamicAttributes", TF2Items_CopyDynamicAttributes },
	{ "TF2Items_CopyCEconItemView", TF2Items_CopyCEconItemView },
	{ "TF2Items_SetAttributeNew", TF2Items_SetAttributeNew },
	{ NULL,							NULL }
};
...
static cell_t TF2Items_SetItemId(IPluginContext *pContext, const cell_t *params)
{
	CEconItemView *item = (CEconItemView*)params[1];

	item->m_iItemID = params[2];
	item->m_iItemIDHigh = params[3];
	item->m_iItemIDLow = params[4];
	return 1;
}

static cell_t TF2Items_GetItemId(IPluginContext *pContext, const cell_t *params)
{
	char* servername;
	sm_sendprop_info_t info;

	pContext->LocalToString(params[1], &servername);

	if (!gamehelpers->FindSendPropInfo(servername, "m_Item", &info))
	{
		return pContext->ThrowNativeError("Failed to get offset for %s::m_Item", servername);
	}

	if (info.actual_offset < 1)
	{
		return pContext->ThrowNativeError("Invalid offset (%i) for %s::m_Item", info.actual_offset, servername);
	}

	CBaseEntity *pEntity = gamehelpers->ReferenceToEntity(gamehelpers->IndexToReference(params[2]));
	if (!pEntity)
	{
		return pContext->ThrowNativeError("Invalid entity %i", params[2]);
	}

	CEconItemView *item = (CEconItemView*)((unsigned char *)pEntity + (int)info.actual_offset);

	//char* item = reinterpret_cast<char*>(pEntity);
	//item += info.actual_offset;
	//reinterpret_cast<CEconItemView*>(item);

	int id = item->m_iItemDefinitionIndex;

	return id;
	//return 1;
}

static cell_t TF2Items_CopyCEconItemView(IPluginContext *pContext, const cell_t *params)
{
	...
	CBaseEntity *pEntity2 = gamehelpers->ReferenceToEntity(gamehelpers->IndexToReference(params[3]));
	if (!pEntity2)
	{
		return pContext->ThrowNativeError("Invalid entity %i", params[3]);
	}

	CEconItemView *olditem = (CEconItemView*)((unsigned char *)pEntity + (int)info.actual_offset);
	CEconItemView *newitem = (CEconItemView*)((unsigned char *)pEntity2 + (int)info.actual_offset);
...
static cell_t TF2Items_CopyDynamicAttributes(IPluginContext *pContext, const cell_t *params)
{
	char* servername;
	sm_sendprop_info_t info;
	...
	/*for (int i = 0; i < ((olditem->m_NetworkedDynamicAttributesForDemos.m_Attributes.Count() > 28) ? 0 : olditem->m_NetworkedDynamicAttributesForDemos.m_Attributes.Count()); i++)
	{
		uint16 def = olditem->m_NetworkedDynamicAttributesForDemos.m_Attributes[i].m_iAttributeDefinitionIndex;
		float flval = olditem->m_NetworkedDynamicAttributesForDemos.m_Attributes[i].m_flValue;

		//g_pSM->LogMessage(myself, "TF2Items_CopyDynamicAttributes: Index: %i | Value: %f", def, flval); //Not needed

		if (def != 0)
		{
			pScriptedItemOverride->m_Attributes[i].m_iAttributeDefinitionIndex = def;
			pScriptedItemOverride->m_Attributes[i].m_flValue = flval;
			pScriptedItemOverride->m_iCount += 1;
		}
	}*/

	//pScriptedItemOverride->m_iCount += params[4];
	//newitem->m_AttributeList.m_Attributes.CopyArray(pScriptedItemOverride->m_Attributes, pScriptedItemOverride->m_iCount);

	pScriptedItemOverride->m_iCount += params[4];
	newitem->m_AttributeList.m_Attributes = olditem->m_NetworkedDynamicAttributesForDemos.m_Attributes;
	newitem->m_AttributeList.m_Attributes.CopyArray(pScriptedItemOverride->m_Attributes, pScriptedItemOverride->m_iCount);

	return pScriptedItemOverride->m_iCount;
}

static cell_t TF2Items_SetAttributeNew(IPluginContext *pContext, const cell_t *params)
{
	char* servername;
	sm_sendprop_info_t info;
	...
	for (int i = 0; i < sizeof(item->m_AttributeList.m_Attributes); ++i)
	{
		int index = item->m_AttributeList.m_Attributes[i].m_iAttributeDefinitionIndex;
		float val = item->m_AttributeList.m_Attributes[i].m_flValue;

		if (item->m_AttributeList.m_Attributes[i].m_iAttributeDefinitionIndex == 0)
		{
			item->m_AttributeList.m_Attributes[i].m_iAttributeDefinitionIndex = params[3];
			item->m_AttributeList.m_Attributes[i].m_flValue = sp_ctof(params[4]);
			return 1;
		}
		if (item->m_AttributeList.m_Attributes[i].m_iAttributeDefinitionIndex == params[3])
		{
			item->m_AttributeList.m_Attributes[i].m_flValue = sp_ctof(params[4]);
			return 2;
		}
